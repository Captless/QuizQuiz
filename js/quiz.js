...
const FORM_DEMO_QUESTIONS = [
  { question: 'What is the chemical symbol for water?', type: 'multiple', options: ['H2O', 'CO2', 'NaCl', 'O2'], answer: 'H2O' },
  { question: 'The Earth revolves around the Sun.', type: 'truefalse', options: ['True', 'False'], answer: 'True' },
  { question: 'What force keeps objects on the ground?', type: 'dropdown', options: ['Gravity', 'Friction', 'Magnetism', 'Inertia'], answer: 'Gravity' },
  { question: 'Which planet is known as the Red Planet?', type: 'multiple', options: ['Mars', 'Venus', 'Jupiter', 'Saturn'], answer: 'Mars' },
  { question: 'Plants produce oxygen during photosynthesis.', type: 'truefalse', options: ['True', 'False'], answer: 'True' }
];

const SLIDE_DEMO_QUESTIONS = [
  { question: 'What is the capital of France?', type: 'multiple', options: ['London', 'Paris', 'Berlin', 'Madrid'], answer: 'Paris' },
  { question: 'Tokyo is the capital of Japan.', type: 'truefalse', options: ['True', 'False'], answer: 'True' },
  { question: 'What is the capital of Australia?', type: 'multiple', options: ['Sydney', 'Melbourne', 'Canberra', 'Perth'], answer: 'Canberra' },
  { question: 'What is the capital of Egypt?', type: 'dropdown', options: ['Cairo', 'Alexandria', 'Giza', 'Luxor'], answer: 'Cairo' },
  { question: 'Brasília is the capital of Brazil.', type: 'truefalse', options: ['True', 'False'], answer: 'True' }
];
  ...
  document.querySelectorAll('.demo-tab').forEach(function (tab) {
    tab.addEventListener('click', function () {
      ...
      if (tab.dataset.view === 'form') {
        renderFormInDemo();
        document.getElementById('demoUrl').innerHTML = '<span class="url-icon">🔒</span> quikquiz.app/quiz/form';
      } else {
        renderSlideInDemo(demoSlideIndex);
        document.getElementById('demoUrl').innerHTML = '<span class="url-icon">🔒</span> quikquiz.app/quiz/slide';
      }
    });
  });
}
...
function renderFormInDemo() {
  var body = document.getElementById('demoBody');
  if (!body) return;
  body.innerHTML = '';
  FORM_DEMO_QUESTIONS.forEach(function (q, i) {
    body.appendChild(buildDemoCard(q, i));
  });
  var footer = document.getElementById('demoFooter');
  footer.innerHTML = '<span class="demo-footer-label">● Quiz Form</span>';
}

function buildDemoCard(q, i) {
  ...
  text.className = 'demo-question-text';
  text.textContent = q.question;
  card.appendChild(text);

  var list = document.createElement('div');
  list.className = 'options-list';
  var shuffled = shuffleArray([].concat(q.options));

  shuffled.forEach(function (opt) {
    var item = document.createElement('div');
    item.className = 'option-item option-item--demo';
...
function renderSlideInDemo(idx) {
  demoSlideIndex = idx;
  var body = document.getElementById('demoBody');
  if (!body) return;
  var q = SLIDE_DEMO_QUESTIONS[idx];
  body.innerHTML = '';
  body.appendChild(buildDemoCard(q, idx));
  ...
  prevBtn.addEventListener('click', function () { goToDemoSlide(idx - 1); });
  nav.appendChild(prevBtn);

  var counter = document.createElement('span');
  counter.className = 'slide-counter';
  counter.textContent = (idx + 1) + ' / ' + SLIDE_DEMO_QUESTIONS.length;
  nav.appendChild(counter);

  var nextBtn = document.createElement('button');
  nextBtn.className = 'slide-nav-btn';
  nextBtn.textContent = '▶';
  nextBtn.disabled = idx === SLIDE_DEMO_QUESTIONS.length - 1;
  nextBtn.addEventListener('click', function () { goToDemoSlide(idx + 1); });
  nav.appendChild(nextBtn);

  footer.appendChild(nav);

  var label = document.createElement('span');
  label.className = 'demo-footer-label';
  label.textContent = '● Slide View';
  footer.appendChild(label);
}

function goToDemoSlide(idx) {
  if (idx < 0 || idx >= SLIDE_DEMO_QUESTIONS.length) return;
  renderSlideInDemo(idx);
}
